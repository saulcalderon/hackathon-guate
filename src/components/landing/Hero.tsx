...
import { ArrowRight, Play, FileText, Cpu, SplitSquareHorizontal, CheckCircle } from "lucide-react";
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({ repeat: -1, delay: 1 }); // Starts blank for 1s initially
    ...
    // We fade OUT step 4 at the very beginning of the timeline (overlaps with Step 1).
    // On the first loop, step 4 is already invisible. 
    // On subsequent loops, this fades out step 4 as step 1 appears, eliminating the blank space.
    tl.to(step4Ref.current, { opacity: 0, y: -20, duration: 0.4 }, 0)
      // Step 1: Upload (Fade in)
    ...
    tl.to(step4Ref.current, { opacity: 1, y: 0, scale: 1, duration: 0.6, ease: "back.out(1.7)" })
      ...
      // Keep step 4 visible for 2 seconds before the timeline loops back to time 0
      .to({}, { duration: 2 });
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <div className="flex flex-col items-center text-center lg:items-start lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <button className="w-full sm:w-auto px-8 py-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-lg transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 flex items-center justify-center gap-2">
                {t("startComparing")} <ArrowRight className="w-5 h-5" />
              </button>
          ...
          <div className="relative w-full aspect-square md:aspect-video lg:aspect-square max-w-xl flex items-center justify-center perspective-1000 mx-auto">
            ...
            >
                ...
                <div ref={step4Ref} className="absolute inset-x-8 inset-y-8 flex flex-col items-center justify-center bg-slate-50 rounded-2xl border border-slate-200 shadow-xl p-6">
                  <div className="w-16 h-16 rounded-full bg-green-100 text-green-600 flex items-center justify-center mb-4 shadow-sm border border-green-200 winner bg-white">
